...
## 10. Roger Design Tokens

Roger extends the base Vercel design system with a WhatsApp-native chat layer. All base tokens (typography, shadows, spacing, radius) are inherited. Only chat-specific tokens are defined here.

### Brand Colors

| Token | Value | Role |
|-------|-------|------|
| `--roger-green` | `#25D366` | Sent message bubble, active states, Roger accent |
| `--roger-green-dark` | `#128C7E` | Hover states, secondary green |
| `--roger-green-light` | `#DCF8C6` | Outgoing message background (Jeff's bubbles) |
| `--roger-canvas` | `#f0f2f5` | Chat panel background (WhatsApp canvas) |
| `--roger-bubble-roger` | `#ffffff` | Roger's incoming message bubble background |
| `--roger-bubble-shadow` | `rgba(0,0,0,0.08) 0px 0px 0px 1px` | Inherits Vercel shadow-as-border on bubbles |
| `--roger-timestamp` | `#667781` | Timestamp and metadata text |
| `--roger-header` | `#ffffff` | Chat panel header background |
| `--roger-divider` | `#e9edef` | Divider lines, panel separators |

### Roger Avatar

- Shape: circle (50%)
- Size: 40px (standard), 32px (compact)
- Background: `#171717` (Vercel Black)
- Letter: "R" in Geist Mono, 16px, weight 600, `#ffffff`
- No border — shadow-as-border: `rgba(0,0,0,0.08) 0px 0px 0px 1px`

### Chat Bubble Component

**Outgoing (Jeff)**
- Background: `#DCF8C6`
- Radius: `12px 12px 0px 12px` (flat bottom-right — sender corner)
- Padding: 8px 12px
- Font: Geist 15px weight 400, `#171717`
- Max-width: 75%
- Align: right
...
- Radius: `12px 12px 12px 0px` (flat bottom-left — receiver corner)
- Padding: 8px 12px
- Font: Geist 15px weight 400, `#171717`
- Max-width: 75%
- Align: left
...
|----------|-------|
| Split | 50 / 50 |
| Background | `--roger-canvas` (`#f0f2f5`) |
| Panel gap | 1px solid `#e9edef` |
| Panel min-width | 320px |
| Panel header height | 56px |
| Panel header bg | `#ffffff` |
| Panel header shadow | `rgba(0,0,0,0.08) 0px 1px 0px 0px` |
| Scroll area | Full height minus header and input bar |
...
- Height: 56px
- Padding: 8px 16px
- Input field: Geist 15px weight 400, `#171717`, radius 999px (pill), background `#f0f2f5`
- Send button: `#25D366` background, white arrow icon, circle 36px, no border
- Shadow on top: `rgba(0,0,0,0.06) 0px -1px 0px 0px`

### Panel Header

- Left: Roger avatar (40px circle) + name ("Roger") in Geist 16px weight 600 + subtitle in Geist 13px weight 400 `#667781`
- Right: optional status badge (pill, `#DCF8C6` bg, `#128C7E` text, 9999px radius)
- Shadow-border bottom: `rgba(0,0,0,0.08) 0px 1px 0px 0px`

### Roger-Specific Do's and Don'ts

**Do**
- Use `#25D366` only for sent bubbles, active states, and the send button — it's Roger's functional green, not decorative
- Inherit all Vercel shadow-as-border techniques for bubble and panel borders
- Keep Geist as the chat font — consistency with the base system matters
- Use `#f0f2f5` as the chat canvas — it's the one place the white canvas is intentionally broken

**Don't**
- Don't use `#25D366` for headings, body text, or decorative elements
- Don't add heavy drop shadows to bubbles — the WhatsApp aesthetic is flat with subtle ring borders
- Don't use border-radius greater than 12px on chat bubbles — it reads as toy-like
- Don't introduce any new font weights beyond the base three (400, 500, 600)
